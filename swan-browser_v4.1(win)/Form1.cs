...
namespace SwanBrowser7
{
    public partial class Form1 : Form
    {
        ...
        // ── Panels declared here (initialized in Designer) ────────────────
        private Panel _bookmarkBar = null!;
        private Panel _urlPanel    = null!;
        private Panel _loadingBar  = null!;
        ...
        private void SetLoadingState(bool loading)
        {
            ...
            // Thin accent stripe under toolbar while loading
            _loadingBar.Visible = loading;
        }
        ...
        public void ApplyTheme()
        {
            ...
            // URL pill
            _urlPanel.BackColor      = ThemeColors.UrlBg(t);
            txtUrl.BackColor         = ThemeColors.UrlBg(t);
            txtUrl.ForeColor         = ThemeColors.UrlFg(t);

            // Loading bar uses accent colour
            _loadingBar.BackColor    = ThemeColors.Accent(t);
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Let the adblocker warm up from its local cache (fast) before
            // the first tab opens; any subsequent filter-list download runs
            // in the background and is picked up by later tabs.
            await Task.WhenAny(AdBlocker.InitializeAsync(), Task.Delay(600));
        ...
        private void urlPanel_Click(object sender, EventArgs e) => txtUrl.Focus();
        ...
        private void tabControl_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            // Fill the bottom strip on inactive tabs
            if (!selected)
            {
            ...
            // Accent top bar on selected tab
            if (selected)
            {
        ...
        {
            const int btnW = 36, btnH = 36, gap = 5, margin = 8;
            ...
            int urlLeft  = margin + (btnW + gap) * 4 + gap * 2;
            int urlRight = pnlToolbar.Width - margin - (btnW + gap) * 3 - gap;

            if (urlRight > urlLeft + 60)
            {
                int panelH = 36;
                _urlPanel.Location = new Point(urlLeft, (pnlToolbar.Height - panelH) / 2);
                _urlPanel.Size     = new Size(urlRight - urlLeft, panelH);

                // Centre the text vertically inside the pill, with 10px side padding
                txtUrl.Location = new Point(10, (_urlPanel.Height - txtUrl.Height) / 2);
                txtUrl.Width    = _urlPanel.Width - 20;
            }
